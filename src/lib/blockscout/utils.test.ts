...
describe("formatBalance", () => {
  ...
  it("formats decimal numbers correctly", () => {
    ...
    expect(formatBalance(BigInt("1500000000000000000"), 18)).toBe("1.5000");
    // 0.1 tokens (with 18 decimals)
    expect(formatBalance(BigInt("100000000000000000"), 18)).toBe("0.1000");
    // 1.23456 tokens (with 18 decimals) - should round to 4 decimals
    expect(formatBalance(BigInt("1234560000000000000"), 18)).toBe("1.2346");
  });
  ...
  it("handles small decimal values correctly", () => {
    ...
    expect(formatBalance(BigInt("1000000000"), 18)).toBe("0.0000");
    // Very small number should show as 0.0000
    expect(formatBalance(BigInt("1"), 18)).toBe("0.0000");
  });

  it("handles different decimal places correctly", () => {
    // 1.5 tokens with 6 decimals (like USDC)
    expect(formatBalance(BigInt("1500000"), 6)).toBe("1.5000");
    // 1.5 tokens with 8 decimals (like WBTC)
    expect(formatBalance(BigInt("150000000"), 8)).toBe("1.5000");
  });
  ...
  it("handles large numbers correctly", () => {
    ...
    expect(formatBalance(BigInt("1000000000000000000000000"), 18)).toBe("1000000.0000");
  });
});
